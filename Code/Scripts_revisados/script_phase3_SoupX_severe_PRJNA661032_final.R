setwd("/home/Script_SD/CellRanger_output/PRJNA661032/SAMN15967269_v6/SoupX")
#packages
library(Seurat)
library(dplyr)
library(patchwork)
...
#Carrega os dados dos outputs direto do CellRanger, cria o objeto SoupChannel e estima o soup profile
...
SAMN15967269_severe_SpX <- autoEstCont(SAMN15967269_severe_SpX, doPlot=FALSE)
...
SAMN15967269_severe[["percent.mt"]] <- PercentageFeatureSet(SAMN15967269_severe, pattern = "^human----MT-")

#Métricas
feats <- c("nFeature_RNA", "nCount_RNA", "percent.mt")

png("severe_before_filters_SAMN15967269.png")

VlnPlot(SAMN15967269_severe, features = feats, pt.size = 0.1, ncol = 3) +
     NoLegend()
dev.off()
...
png("severe_after_filters_SAMN15967269.png")

VlnPlot(SAMN15967269_severe.filtered.nCount_RNA, features = feats, pt.size = 0.1, ncol = 3) +
     NoLegend()
dev.off()
...
out_test <- as.matrix(SAMN15967269_severe@assays$RNA@counts)
write.table(out_test, file="features_severe_SAMN15967269.tsv", quote=FALSE, sep='\t', col.names = TRUE)
write.csv(SAMN15967269_severe@meta.data, file="metadata_severe_SAMN15967269.csv")
...
not_infected <- rownames(sample_sarscov2_transposta[ rowSums(sample_sarscov2_transposta)==0, ] )

#print rows where all columns are different of zero
infected <- rownames(sample_sarscov2_transposta[ rowSums(sample_sarscov2_transposta)>0, ] )
