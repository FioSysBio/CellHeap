...
png("/scratch/inova-covd19/vanessa.silva/mild_before_filters_liao.png")
...
png("/scratch/inova-covd19/vanessa.silva/mild_after_filters_liao.png")
...
write.csv(counts, file="features_mild_liao.tsv")

#import of seurat filter file
sample <- read.table("features_mild_liao.tsv", sep=",", header=T, row.names = 1)
...
write.table(sample[,not_infected], file= "features_not_infected_mild_liao.tsv", quote=FALSE, sep=',', col.names = TRUE)
